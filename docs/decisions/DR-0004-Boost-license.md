...
In Dok libraries can managed like templates, i.e. specialized and then injected inside the code of the combined-work.
...
The extremely permissive license [Boost Software License v1.0](https://www.boost.org/LICENSE_1_0.txt) will be used, because:
- the library code can be injected inside derived work code without affecting its license
- it is used for C++ STL Boost libraries that are used from many projects in a way similar to Dok usage scenario
...
Dok can use external libraries using different licenses, and in some cases it can also analyze and import their code directly in the derived work source code. So Dok MUST include a licensing management tool analyzing the license of used libraries, and how they are used inside the derived product, for inferring:
...
DokMelody documentation that is not part of the source code will be released under [CC-BY-4.0](https://creativecommons.org/licenses/by/4.0/) for maximizing reuse. It is a well known license, used from big projects like WikiPedia. 
...
LGPLv3+ enforces a very nice and fair "right to repair", i.e. the user of a product X using a LGPL library L MUST be able to create a new version of X using a new/better/patched version of library L.
...
Using LGPLv3+ for DokMelody IDE tools can introduce annoying license complications. If DokMelody IDE transforms the source code injecting some LGPLv3+ source code using a "copy-analyze-transform-and-paste" editor library, then:
