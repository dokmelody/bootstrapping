...
MultiKore is a DSL used for expressing computational heavy parallel algorithms, with an hopefully linear speedup. It uses the nested data parallelism approach:
 - code is like pure functional code, (e.g. [[map-and-reduce)]] and very similar to Dok, so the paradigm is rather natural for programmers knowking functional programming
