...
An excetpion is not managed by a =try-else= branch because it is not an expected branch of computation but it is an unexpected error to manage.

Dok joins the approach of Eiffel cite:meyer1997object with the approach of the Actor model cite:armstrong_making_nodate.  
...
Another valid approach is rethrowing the exception to the parent block. The rethrowed exception can be a generic error exception or an exception with more details. The parent block will receive a stack of exceptions, and it can use this information mainly for log-reporting.

Exceptions in Dok like in Eiffell can not be ignored/masked.
...
The restoring of the system is a very delicate and difficult thing to do because exceptions can happen in any point of the code, and so the variants to take in consideration are too much. For solving this problem Dok follows these approaches:
- in case of values they can be restored automatically using the nested transaction semantic
- in case of impure actions theirs rollback is managed by Dok compiler plugins that can analyze the code and according the used instructions, the developer annotations and the developer customizations can apply the correct strategy

Also retry or rethrow strategy is decided using Dok compiler plugins. 
...
The Dok code is mainly free from exception handling branch, and all these details are managed using compiler plugins and meta annotations.

The rationale it is that in case of exceptional problems there are usually well defined and generic strategies to follow (e.g. rollback databases connections, retry a network connection a certain number of times, etc..) and these strategies can be coded in one place, instead of being repeated in all the code. This is a form of [[#AOP]].

TODO the effective implementation and library support is postponed
* Impure statements
  :PROPERTIES:
...
Resources are initializated, used and released. In case of exceptions they can be released in the normal way or in some ad-hoc way according the type of exceptions and the operations done on the resource before the exception.

Like in case of [[#exceptions]] the resources are finalized both in normal case and in case of exception from custom compiler plugins. This because the resource logic management is usually common for every type of resource, or type of application and it can be specified only one time. This is a form of [[#AOP]].  

Concretely resources in Dok are explicitely acquired but implicitely released.

TODO the details will be postponed
** References
   :PROPERTIES:
