...
#+END_SRC
** Overriding
Nested type variants can override properties and methods of parent type like in normal object-oriented languages. Also types of the methods can be redefined during overriding (see [[type_conformance]]).

For making explicit that a property or method is redefined the syntax ~:~ and ~->~ is used instead of ~::~ and ~-->~.
...
~super~ is also used for acessing the ~self~ property inside a nested lexical scope. See this example:
...
~this~ is a reserved word, and ~this(x, y)~ calls the current function.
** Binary operators
...
In Dok functions can be passed as arguments. This allows a lot of useful functional programming patterns. 
...
Some example of higher-order functions usage:
 ...
 Assert {
   ...
   Assert {
     ...
     # Instead of `({...})` an alternative syntax for specifying functions is 
...
- also resources are managed in the same way, see [[#resource_management]]
...
So the Dok code is mainly free from exception handling branch, and all these details are managed using compiler plugins and meta annotations. The rationale it is that in case of exceptional problems there are usually well defined and generic strategies to follow (e.g. rollback databases connections, retry a network connection a certain number of times, etc..) and these strategies can be coded in one place, instead of being repeated in all the code. This is a form of [[#AOP]]. The code become both more compact and more robust.
* Impure statements
  :PROPERTIES:
