...
** Tuples
   ...
   :CUSTOM_ID: tuple
   :END:
A tuple is a [[#record]] having implicit default field names.
...
#+END_SRC
** Containers as values (i.e. as persistent data structures)
...
Like in OOP and FP, concrete types can support abstract nterfaces representing abstract concepts.
...
#+END_SRC
** Compact type variants
  :PROPERTIES:
  :CUSTOM_ID: compact-type-variants 
  :END:
Every type variant is a distinct type, but sometime these types can have no attributes or only one attribute, so they are rather compact. Dok allows to use a compact syntax for these types. 
...
$Assert{ 
  ...
  p1::= MathPrecision/Infinite
  #:NOTE: `MathPrecision/Infinite` is a type without attributes. 
  #       Its only information consists in its type-tag,
  #       and this is completely fair and coherent with Dok type semantic.

  p2 == MathPrecision/Infinite
  #:NOTE: the type of `p2` can be used in tests, like it is a value.

  p2 == Infinite
  #:NOTE: it is known at compile-time that `p2.is(MathPrecision)`
  #       so this short expression is perfectly clear,
  #       and more readable than long version `MathPrecision/Infinite`.

  p2::= MathPrecision/Finite(4)
  #:NOTE: `MathPrecision/Finite` type has only one attribute, 
  #       so it can remain unamed.  

  p2 == Finite(4)
  #:NOTE: again a rather compact notation

  p2.x0 == 4
  p2.get(0) == 4
  #:NOTE: types with unamed attributes follow
  #       the same syntax and semantic of tuples.
} 
#+END_SRC
** Compact types
They have the same syntax and semantic of [[compact-type-variants]]. They are more used inside type-variants than as standalone types.
** Overriding
...
An interface can require that the type supporting it will support also one or more other interfaces. Doing so interfaces forms an hierarchy where more constraints and features are added to every layer. 
...
#+END_SRC
*** Interface instances
A concrete type can support one or more interfaces. The syntax is similar to the case of interfaces hierarchy, but in this case there is a concrete type and an interface, instead of two interfaces.

#+BEGIN_SRC fundamental :tangle yes
 Int -> Comparable ( 
   #:NOTE: ``->`` is used instead of ``-->`` because `Int` is not a new type,
   #       but an already existing type, that we are extending
   #       with the support of a new interface.
...
** Optional arguments
  :PROPERTIES:
  :CUSTOM_ID: function-optional-arguments
  :END:
Dok tries to join in a unique way the concepts of
- functions with optional arguments having a default value
- classes with static fields (i.e. field value is unique for each instannce of the class)
- dependency injection

The idea is that:
- diferent functions can have the same set of optional arguments
- a value of an optional argument should be set only one time from a caller and used every time functions sharing the same set of optional arguments are called

Viewed in this way the semantic is similar to using global variables contaning the params values, or better having a shared environment, or better having a shared environment but with a predictable lexically-scoped semantic like in case of =with= used for [[dependency_injection]].
...
$PseudoCode{
  ...
  Invoice --> (
    ...
    showTotal --> String {
      self.total.show(withPrecision:= $locale.currencyPrecision)
      #:NOTE: ``currencyPrecision`` is like an otpional ever accessible argument of the function
    }
  )

  do {
    with [
      $locale.currencyPrecision = 2
      $locale.language = "it"
    ]
    i::= Invoice(total:= 92.758)
    !print i.showTotal
  }
}
#+END_SRC

** Function arguments are a record
The arguments of a function can be seen as a record in Dok. A function can be called specifying some code returning a compatible record.

#+BEGIN_SRC fundamental :tangle yes
$Assert{

  Int.sum3(x::Int, y::Int) --> Int {
    self + x + y
  }

  args::= (x:= 1, y:= 1)
  1.sum3(1, 1) == 1.sum3{args} == 3
...
The task is: replace the tree with a
similar tree, but where each node value is replaced with the minimum node
value of the original tree.

The AG solution is based on:
- a bottom-up attribute for calculating the minimum value of eachh subtree
- a top-down attribute telling to each node which is the global minimum value found in the entire tree
- a rewrite attribute with a subtree similar to the previous tree, but with the minimum value used in each `Tree/Tip` node
...
$Assert{
  ...
  Tree -> (
    ...
    globalMin <-- T {
      ...
      #       You can imagine a top-dow attribute like an implicit
      #       parameter passed to any function. It is sort of 
      #       always accessible environment.

      (self.globalMin, self.min)*.minim
      #:NOTE: the global minim value to pass to each subtree is the minimum
      #       of the value found from the parent node and passed to this node
      #       using the `globalMin` attribute, and the minimu value found in
      #       children of the node, using so bottom-up attributes.
   
    }

    globalMin <--init-- {
        self.min
	#:NOTE: each top-down attribute starts with an initial value in the case
	#       this is the first node of the hierarchy.
    }
  ...
  Tree -> (
    withGlobalMin --> Self {
      ...
      $RewriteRule
      #:NOTE: apply semantic of rewrite rules.
      #       In particular replace each node with trees with replaced elements.
      #       Because Dok is a value based language, a new copy of the tree
      #       must be created explicitely. The rewrite rule avoids the repetition
      #       of simple clone operations.
    }

    /Tip -> (
      withGlobalMin -> Self { 
       Tip(value:= self.globalMin)
       #:NOTE: replace this node with a version having the global minimum found value.
      }
    )

    #:NOTE: the ``$ReriteRule`` annotation create this boilerplate function in automatic
    # > /Fork( withGlobalMin -> (left:= self.left.withGlobalMin, right:= self.right.withGlobalMin))
  )
  ...
  #:NOTE: we are applying `globalMin` to a new tree value, 
  # so the visit start from the beginning again, starting with
  # the ``init`` rule o the attribute.

  tree1.withGlobalMin == Tree(Fork(Tip(1), Fork(Tip(1), Tip(1))))
} 
#+END_SRC
** Bootm-up attributes
...
A collection attribute keep collecting new values during the visiting of the tree. It can be implemented using normal bottom-up attributes, but the pattern is often used, so they are also a special type of attribute. They start always as an empty collection, and then every rule specify which elements to add to the collection. 
...
** Nested DSLs
Dok code can be composed of many nested/embedded DSLs, e.g.:
- a source language ~S~
- a nested/embedded/guest language ~E~
- and so on, forming arbitrarly nested hieararchies of DSL code fragments
** Source DSL to eval
#+BEGIN_SRC fundamental :tangle yes
$PseudoCode{
  Int.add_v1(x::Int) --> Int $Dok{
    #:NOTE: a function returning a Dok `Int` value, 
    #       and specified using `Dok` DSL.
 
    self + x
  }

  # `add_v1` can be obviously defined omitting the ``$Dok`` annotation,
  # because inside `Dok` code, the ``$Dok`` is implicit:

  Int.add_v2(x::Int) --> Int { 
    self + x 
  }
  ...
  #:NOTE: `x` has Dok type `Int` and its value is calculated evaluating 
  #       a C-like code. 
}
#+END_SRC
** Embedded DSL to eval
...
$PseudoCode{
   ...
   Int.div(x::Int) --> Int {
     $Require{
       #:NOTE: this is code specified in `Require` DSL
       #       and converted from compiler into `Dok` code testing
       #       the property at run-time or in other cases 
...
A DSL can be used for defining new types.
#+BEGIN_SRC fundamental :tangle yes
@PseudoCode{
  BinaryNumber --> $PEG(
    ~ <-- (Zero / One)+
  ...
  #:NOTE: `BinaryNumber` is a Dok type defined using a syntax similar to Packaret Expression Grammar.
  #       The compiler convert the type definition from PEG syntax to some valid Dok syntax.
  
  b::BinaryNumber
  b:= "10".BinaryNumber
  b(0).is(BinaryNumber/Zero)
  b(1).is(BinaryNumber/One)
}
#+END_SRC
** Generating DSL values at run-time
Values can have a DSL that is an ad-hoc syntax for representing them. These values can be generated from this format, parsing a string. This can be done at run-time.

#+BEGIN_SRC fundamental :tangle yes
@PseudoCode{

  "6.div(2)".Dok.is(Dok)
  #:NOTE: this is a fragment of Dok code
  ...
  # if the run-time supports Dok compilation and eval then we can do

  c.eval == 3 
}
#+END_SRC
** Generating DSL values at compile-time
DSL code can be used for defining values. The DSL define both a type and a DSL.
#+BEGIN_SRC fundamental :tangle yes
$PseudoCode{
  ...
  s::= $String(
    The DSL of a string is the content of the string.
    It can refers to other values using unquoting, like x = ${x.toString}.
  )

  # Obviously also basic types like Int, admit this
  z::= $Int(5)
...
#+END_SRC
** Annotations and meta-info
Annotations are additional meta-info associated to a piece of code, that can suggests proper behaviour to compilers processing the code.

An annotation generate some information that can be recognized or discarded from compilers. So the information is not mandatory for the execution of the code.
#+BEGIN_SRC fundamental :tangle yes
$PseudoCode{

   Int.fun3(x::Int) {
     $Inline
     #:NOTE: this is an annotation sent to the compiler.
     #       So it is like meta-info hints.

     self + x
   }
 
   Int.anotherFun(x::Int) --> Int {
     $SomeMetaInfoProp(someAttr: "some tag", anotherAttr: 5)
     #:NOTE: this is meta info applied to `anotherFun` lexical-scope,
     #       having additional attributes.
   }

}
#+END_SRC
** Macro expansions
A function can generate code in the host DSL. The function acts like a macro generating code to compile.

The code returned from the function must be the same of the host DSL, or some DSL recognized from the compiler.

#+BEGIN_SRC fundamental :tangle yes
$PseudoCode{

   $someMacroExpansion(someParam)
   #:NOTE: call `someMacroExpansion` at compile-time that will generate some code
   #       in the current DSL.
}
#+END_SRC
** Referencing entities defined in another DSL
An embedded DSL can define variables and these variables can be used inside the host DSL. The semantic of these variables depends from the compiler plugin managing the compilation and it depends from the host DSL, the embedded DSL and the context. 

Usually it should be rather intuitive figuring out the meaning of the code (e.g. interaction between an imperative language and a relational database), but the compiler can complete all boiler-plate details and thanks to global analysis of code can optimize some access patterns.

These "foreign" entities are prefixed by ~$~ when their semantic is different from the semantic of current DSL. Doing so it became more explicit that there are hidden details, managed by the compiler. When the semantic of the two DSL is the same or very similar, there is no usage of ~$~ prefix.

The same ~$~ prefix is used for referencing in an embeded DSL entities defined in the host DSL, and having a different semantic respect the embeded DSL.
...
$PseudoCode{
  ...
  $DB [
    Person --> ( 
      name::$String
      #:NOTE: we are referencing the String type defined in ``Dok``
      #       that is a value and not an entity, like in case of 
      #       types defined inside ``DB`` and so it make sense
      #       using ``$String``.

      father::Person?
      #:NOTE: in ``DB`` semantic types are references, 
      #       and we have references instead of values.

      mother::Person?
    )

    mzan::Person(name: "Massimo Zaniboni")
  ]

  !IO.println($mzan.id ++ ": " ++ $mzan.name)
  #:NOTE: ``$mzan`` is a variable defined in a guest DSL, and it has a semantic different from ``Dok``
  #       (i.e. it is a reference in this case). 
  #       The compiler must manage it in a special way, and it is useful making explicit also in the code.
 
}
#+END_SRC
** Injecting embeded DSL code in the same lexical-scope of the host DSL 
A fragment of DSL code can be part of the host DSL lexical-scope, but expressed using a different DSL.
#+BEGIN_SRC fundamental :tangle yes
$PseudoCode{
  x::= 5
  ...
  y == 6 
  #:NOTE: `y` is visible inside the paren lexical-scope, 
  #       because the code inside ``[...]`` is at the same lexical-scope. 
}
#+END_SRC
...
*** User customizable compilation passes 
 Dok code can be composed of nested DSL, e.g. a source/host language ~H~, a target language ~T~, an optional embeded language ~E~.

 Code is analyzed and transformed to ~T~ DSL using a series of collaborating and user definable compilation passes.

 A compilation pass can translate ~E~ directly into ~T~, or it can be helped by DSL compilers translating first ~E~ into ~H~ DSL.

 Compilation pass are ordered. The order of passes is important, because it can change the semantic or the produced code ~T~.
 Passes are executed one or more time, iteratively, until a fix point is reached. In this way 
 a compiler ~C~ can produce also some intermediate code in other DSL, and these fragment of code will be further transformed.

 TODO details will be fixed during development and real-world usage. It depends also from:
 - the real power of attribute-grammars
 - lesson learned studying other metaprogramming and compilation based systems
*** Dokk
 Dokk is the Dok Kernel Language. Dokk is a simplified variant of Dok with strict semantic, no nested transactions and attribute grammars. Its run-time costs can be easily understood.

 Dokk can be the target of many compilation pass involving imperative code, because it is high-level, but with predictable run-time cost.
** Dok is not an homoiconic programming language
Dok language thanks to attribute-grammars and other logical rules is good for writing compilers, but contrary to Lisp it is not an homoiconic programming language, because the syntax of Dok code is different from the data structure used for representing code. 

This is not a big disavantage in Dok, because in any case code is represented using abstract-syntax-tree (AST) that are a good fit for attribute-grammars, and the natural hierarchical data format of Dok. An homoiconic syntax can simplify simple macro and code processing, but complex transformations requires a global view of the AST.
** Compile-time only metaprogramming
Dok does not implement all the features of a generic metaprogramming language, because it supports only compile-time meta-programming, i.e. Dok applications are usually shipped without reflection and meta-information about their code.

Only the Dok compiler and optionally the DokMelody IDE have access to the code of the application, but this is done using normal Dok constructs on data that in this case represents code. So Dok has a simplified semantic respect programming languages with metaprogramming at run-time.
...
In Dok dependency injection and contexts can be used for modifying default options/parameters of services but also of normal functions.
** The ~with~ modifier
Rollbackable actions prefixed with ~with~ will be rollbacked at the exit of the scope owning them. So the ~with~ can configure something, and then restore to original params at the end.
 ...
 $Assert{
   ...
   do {
     with [
       x::= 2
       #:NOTE: the effects will be discarded outside the scope of parent =do { .. }=
     ]
     x == 2
   }
   x == 1
...
~with~ can be used also for customizing methods of a type inside some part of the code, using ad-hoc code.

The advantage of ~with~ semantic are that in- effects depend from the lexical scope of the ~with~ instruction and not from the dynamic scope, so they are more predictable (i.e. a property of code and not of code execution).

** Context
Functions with optional arguments use contexts. See [[function-optional-arguments]].
...
$Assert{
  ...
  # but without creating a lexical scope for compiled code,
  # i.e. the lexical scope is only for the meta-programming annotation.
  # For this reason there is no identation.
...
In Lisp arguments of a function call are elements of a list and they are manageable both a compile-time and run-time without difference. In Dok you can not change at run-time (at least easily as in Lisp) the name of the function to call (that in Lisp is the first argument of the list), but you can pass arguments to a function passing a record, that is manegeable at run-time in a similar way to Lisp.
...
Lisp is homoiconic because Lisp abstract syntax tree is also Lisp code. Dok instead has a syntax that is different from its abstract syntax tree representation. In any case Dok has useful costructs for analyzing and transforming hierarchical data (i.e. code). 
** Haskell
...
As said also Dok tries to join functional and object oriented type systems concepts using compact type variants in functional-like way. On the contrary the usual OOP approach is used for interfaces, because a concrete type suppos an interface case by case.
