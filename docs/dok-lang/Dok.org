#DATE: 2019-06-02
#+TITLE: Dok Programming Language v0.1.3
...
- meta-annotations and compiler plugins can be used for deriving automatically low-level code from high-level code
- a complex problem can be partitioned in rather orthogonal aspects

Summing up: in Dok source code is a first class citizen so there can be user-customizable compiler plugins for analyzing and transforming it or for injecting (low-level) functionalities.
...
The Knil schema and queries are meant to be high level specifications, then according application requirements and used queries the Knil compiler with the help of the user can derive a concrete database.
...
~@Require~ is used instead of ~@Ensure~ because the constraint is seen from the point of view of the user adding info to the database. But ~@Ensure~ can be used because in this case is perfectly compatible, i.e. it is same requirement seen from the point of view of the reader of the database. 
...
Knil types are tagged with`$SomeKinlType` inside Dok, and Dok types are tagged with `$SomeDokType` inside Kniil. The tag is important for the user reading the code because they have different semantic. It is important also for the the Knil compiler that can manage them in ad-hoc way.

Knil types are seen as containers of tuples in Dok, so they can be queried using a [[#map-reduce]] proramming model.
** Weak vs strong references
In OOP a weak reference is a link between two distinct objects/entities, while a strong reference is when an object is part of another object. A typical distinction is when you clone an object: you copy weak references without cloning the referenced object, but you clone an object linked with strong a reference to the original object to clone, because it is part of it, so you had to clone also its parts. 

In case of Knil and Dok we have clear distinctions between strong and weak references:
- every tuple inside a Knil table is an entity and every foreign key pointing to another entity is for sure a weak reference, i.e. a link to a distinct entity
- every compound value (e.g. XML/HTML/JSON documents, etc..) is composed of parts (i.e. strong references)
- a compound value can have references to Knil entities, in particular in case of a noSQL data processing, but these are managed as values like ~Ref(SomeForeignKey)~ and so they act like weak references to the referenced entity
** Knil functions
   :PROPERTIES:
...
According cite:meijer_co-relational_nodate coSQL are the dual of RM, i.e. many properties are the opposite of the relational counterpart. There are not conflicts between the two models because instead they complement each other. Many benefit of one model are not present in the other and vice-versa. For example:
