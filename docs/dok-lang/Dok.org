...
* Functions
  ...
  :CUSTOM_ID: functions
  :END:
In Dok a function is a referential transparent method of an object which value depends from the ~self~ object, the arguments of the function and the function body. 

In Dok every function must be a method of a type, so it must have the ~self~ object, i.e. there can not be functions with only arguments, but without a ~self~ object.
...
** Pure actions
   ...
   :END:
 A pure action is a rollbackable action (according the [[nested-transaction-semantic]]) that can change the properties of the `self` object, the variables of its parent lexical scope and that can return a value of the declared type. If no return type is specified, then the action return no value.
...
** Nested actions
  ...
  :END:
A nested actions is like a [[nested-function]] but it can change the variables of the parent function, and the properties of the object of the parent function.
